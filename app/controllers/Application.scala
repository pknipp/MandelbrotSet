...
class Grid(val size: Double, val nxOverTwo: Int, val maxIter: Int, val mag: Int, val c: Complex) {
  ...
  def fromDom(z: Complex): String = {
    val zNew = z.mul(new Complex(2 / size, 0)).add(new Complex(-2.0, -2.0)).mul(new Complex(pow(2, -mag).toDouble, 0.0)).add(new Complex(c.x, c.y))
    zNew.x + ", " + zNew.y
  }
}
